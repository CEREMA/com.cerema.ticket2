Officer = {
	login : function(profile,auth_type,cb)
	{
		if (auth_type=="cas") {
			...
			Officer.using('db').store('bpclight','select agents.fax,agents.kage,agents.kuni,agents.ksub,nom,prenom,unites.libunic,unites.kets,subdis.libsubc from agents join unites on unites.kuni=agents.kuni join subdis on subdis.ksub=agents.ksub where agents.kage in (select kage from mela where libmela="'+mail+'")',function(err,result){
				if (!err) {
					var response={
