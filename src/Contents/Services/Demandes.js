...
Demandes = {
	...
	myclitickets: function(o,cb) {
		var db=Demandes.using('db');
        
		if (o.profil.indexOf("SII")>-1) {
			// profil SII, voit tout le monde
			db.model('infocentre','select * from ticket where cli='+o.id+' and state<>-1 and state<5 order by date_depot desc',cb);
			return;
		};
		if (o.profil.indexOf("GEST")>-1) {
			// profil SII, voit tout le monde
		...
		if (o.profil.indexOf("CLI")>-1) {
			...
			db.query('bpclight',"select kage from agents where kuni in (select kuni from agents where kage='"+o.uid+"') and agents.actif=1", function(e,r) {
				...
				db.model('infocentre','select * from ticket where agent in ('+KAGE.join(',')+') and state<>-1 and state<5 and cli='+o.id+' order by date_depot desc',cb);
			});
			return;
		}
	},
